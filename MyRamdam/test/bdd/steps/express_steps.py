...
def and_the_various_feelings_are_shown(step):
    assert False, 'This step must be implemented'


@step(u'When the user chooses a feeling')
def when_the_user_chooses_a_feeling(step):
    assert False, 'This step must be implemented'


@step(u'Then the feeling is recorded')
def then_the_feeling_is_recorded(step):
    assert False, 'This step must be implemented'
